    ...
    <div className="fixed inset-0 bg-black z-[60] flex flex-col">
      ...
      <div className="flex-1 relative flex items-center justify-center p-4 overflow-hidden bg-black">
        {currentFrame.image_base64 ? (
            ...
            <div className="flex flex-col items-center gap-4 text-zinc-600">
                ...
                <span className="text-xs uppercase tracking-widest">Carregando Imagem...</span>
            </div>
        )}
        
        <button onClick={handlePrev} className="absolute left-4 p-4 text-zinc-500 hover:text-red-600 transition"><ChevronLeft size={48} strokeWidth={1} /></button>
        <button onClick={handleNext} className="absolute right-4 p-4 text-zinc-500 hover:text-red-600 transition"><ChevronRight size={48} strokeWidth={1} /></button>
      </div>
      ...
      <div className="bg-black p-8 pb-10 border-t border-zinc-900">
        <div className="max-w-4xl mx-auto text-center">
          ...
          {currentFrame.camera_move && <span className="inline-block bg-red-600 text-white text-[10px] font-bold px-2 py-1 mb-4 uppercase tracking-widest">{currentFrame.camera_move}</span>}
...
const ProjectList = ({ projects, onSelect, onCreate, onDelete, onUpdate, loading, onRefresh }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (coverImage) coverBase64 = await convertFileToBase64(coverImage);
    const projectData = { title, description: desc, cover_image: coverBase64 };
    if (editingProject) onUpdate(editingProject.id, projectData);
    else onCreate(projectData);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-end mb-12 border-b border-zinc-900 pb-6">
        ...
        <div className="flex gap-4">
            <button onClick={onRefresh} className="bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white px-4 py-3 font-bold flex items-center gap-2 transition text-sm tracking-wide uppercase border border-zinc-800" title="Recarregar Projetos">
                <RefreshCw size={18} className={loading ? "animate-spin" : ""} />
            </button>
            <button onClick={openCreateModal} className="bg-red-600 hover:bg-red-700 text-white px-6 py-3 font-bold flex items-center gap-2 transition text-sm tracking-wide uppercase shadow-[0_0_15px_rgba(220,38,38,0.3)] hover:shadow-[0_0_25px_rgba(220,38,38,0.5)]">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-950 border border-zinc-800 w-full max-w-md p-8 shadow-2xl relative overflow-hidden">
            ...
            <h2 className="text-2xl font-bold text-white mb-6 tracking-tight">{editingProject ? 'EDITAR PROJETO' : 'NOVO PROJETO'}</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
                 ...
                 <div className="relative w-full h-32 bg-black border border-zinc-800 flex items-center justify-center overflow-hidden group cursor-pointer hover:border-red-600 transition-colors">
                    {coverPreview ? (
                        <>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center"><span className="text-xs font-bold text-white uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-md">Trocar Imagem</span></div>
                        </>
                    ) : (
                        <div className="flex flex-col items-center text-zinc-700 group-hover:text-red-600 transition-colors"><ImageIcon size={24} className="mb-2" /><span className="text-[10px] font-bold uppercase tracking-widest">Upload Capa</span></div>
                    )}
                    <input type="file" accept="image/*" onChange={handleCoverChange} className="absolute inset-0 opacity-0 cursor-pointer" />
                 </div>
              </div>
              <div><label className="block text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">TÍTULO</label><input autoFocus type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-black border border-zinc-800 p-4 text-white focus:border-red-600 outline-none transition-colors placeholder:text-zinc-700" placeholder="DIGITE O NOME DO PROJETO" /></div>
              <div><label className="block text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">DESCRIÇÃO (OPCIONAL)</label><textarea value={desc} onChange={e => setDesc(e.target.value)} className="w-full bg-black border border-zinc-800 p-4 text-white focus:border-red-600 outline-none h-24 transition-colors placeholder:text-zinc-700 resize-none" placeholder="Detalhes sobre o filme..." /></div>
              <div className="flex justify-end gap-4 pt-2">
                ...
                <button type="submit" className="bg-red-600 hover:bg-red-700 text-white px-8 py-3 text-xs font-bold uppercase tracking-widest transition">{editingProject ? 'Salvar' : 'Criar'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {projects.length === 0 && !loading ? (
        <div className="flex flex-col items-center justify-center py-32 border border-dashed border-zinc-900 bg-zinc-950/50">
          <div className="w-16 h-16 bg-zinc-900 flex items-center justify-center mb-6 rounded-full"><Film size={32} className="text-zinc-700" /></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map(project => (
            <div key={project.id} onClick={() => onSelect(project)} className="group bg-zinc-950 border border-zinc-900 hover:border-red-600/50 cursor-pointer transition-all duration-300 relative overflow-hidden min-h-[250px] flex flex-col">
              {project.cover_image && (
                  <div className="absolute inset-0 z-0">
                      <img src={project.cover_image} alt="Cover" className="w-full h-full object-cover opacity-30 group-hover:opacity-50 transition-opacity duration-500 grayscale group-hover:grayscale-0" />
              ...
              <div className="relative z-10 p-8 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-auto">
                    <div className={`transition-colors duration-300 ${project.cover_image ? 'text-white' : 'text-zinc-700 group-hover:text-red-500'}`}><Film size={24} /></div>
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={(e) => openEditModal(project, e)} className="text-zinc-400 hover:text-white transition bg-black/50 p-2 rounded-full backdrop-blur-sm" title="Editar"><Edit3 size={16} /></button>
                      <button onClick={(e) => { e.stopPropagation(); onDelete(project.id); }} className="text-zinc-400 hover:text-red-500 transition bg-black/50 p-2 rounded-full backdrop-blur-sm" title="Excluir"><Trash2 size={16} /></button>
                    </div>
                  </div>
                  <div className="mt-8">
                      ...
                      <div className="flex items-center gap-2 text-[10px] font-mono text-zinc-500 uppercase tracking-widest border-t border-zinc-800/50 pt-4"><Calendar size={12} /><span>{new Date(project.created_at).toLocaleDateString()}</span></div>
                  </div>
              </div>
            </div>
          ))}
...
const FrameEditor = ({ isOpen, onClose, onSave, initialData, isSaving }) => {
  const [formData, setFormData] = useState({ title: '', description: '', prompt: '', cameraMove: '', image: null, imagePreview: null });

  useEffect(() => {
    if (initialData) {
      setFormData({ title: initialData.title || '', description: initialData.description || '', prompt: initialData.prompt || '', cameraMove: initialData.camera_move || '', imagePreview: initialData.image_base64 || null, image: null });
    } else {
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-zinc-950 border border-zinc-800 w-full max-w-5xl h-[85vh] shadow-2xl flex flex-col md:flex-row overflow-hidden relative">
        ...
        <div className="w-full md:w-1/2 bg-black flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-zinc-900 relative group">
          {formData.imagePreview ? <img src={formData.imagePreview} alt="Preview" className="w-full h-full object-contain p-8" /> : <div className="text-zinc-700 flex flex-col items-center"><ImageIcon size={64} strokeWidth={1} className="mb-4" /><p className="text-xs uppercase tracking-widest">Nenhuma imagem selecionada</p></div>}
          <label className="absolute inset-0 flex items-center justify-center bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
            ...
            <div className="border border-red-600 px-6 py-3 text-red-500 text-xs font-bold uppercase tracking-widest hover:bg-red-600 hover:text-white transition">{formData.imagePreview ? 'Trocar Imagem' : 'Selecionar Frame'}</div>
          </label>
        </div>
        <div className="w-full md:w-1/2 p-8 flex flex-col bg-zinc-950 overflow-y-auto">
          <div className="flex justify-between items-center mb-8 border-b border-zinc-900 pb-4">
            <h2 className="text-xl font-bold text-white tracking-tight uppercase">{initialData ? 'EDITAR FRAME' : 'NOVO FRAME'}</h2>
            <button onClick={onClose} className="text-zinc-500 hover:text-white transition"><X size={24} /></button>
          </div>
          <form onSubmit={handleSubmit} className="flex-1 space-y-6">
            <div><label className="block text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">CENA / TÍTULO</label><input type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full bg-black border border-zinc-800 p-3 text-white focus:border-red-600 outline-none transition-colors" placeholder="Cena 1 - O Monolito" /></div>
            <div><label className="block text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">DESCRIÇÃO</label><textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full bg-black border border-zinc-800 p-3 text-white focus:border-red-600 outline-none h-24 resize-none transition-colors" /></div>
            <div className="grid grid-cols-2 gap-4"><div><label className="block text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">MOVIMENTO</label><input type="text" value={formData.cameraMove} onChange={e => setFormData({...formData, cameraMove: e.target.value})} className="w-full bg-black border border-zinc-800 p-3 text-white focus:border-red-600 outline-none transition-colors text-sm" placeholder="ZOOM IN" /></div></div>
            <div><label className="block text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">PROMPT ORIGINAL (IA)</label><textarea value={formData.prompt} onChange={e => setFormData({...formData, prompt: e.target.value})} className="w-full bg-black border border-zinc-800 p-3 text-zinc-300 font-mono text-xs focus:border-red-600 outline-none h-32 resize-none transition-colors" /></div>
            <div className="pt-6 flex justify-end gap-4 border-t border-zinc-900 mt-auto">
              ...
              <button type="submit" disabled={isSaving} className="bg-red-600 hover:bg-red-700 text-white px-8 py-3 text-xs font-bold uppercase tracking-widest transition disabled:opacity-50 flex items-center gap-2">{isSaving && <Loader2 className="animate-spin" size={14} />} Salvar</button>
            </div>
          </form>
        </div>
      </div>
...
const FrameCard = ({ data, onDelete, onEdit, index, onDragStart, onDragEnter, onDragEnd, onSetCover }) => {
  ...
  const handleCopyPrompt = () => { if (!data.prompt) return; copyToClipboard(data.prompt); setCopied(true); setTimeout(() => setCopied(false), 2000); };
    ...
    >
      <div className="relative aspect-video bg-black overflow-hidden" onClick={() => onEdit(data)}>
        {data.image_base64 ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-zinc-800 gap-2">
              ...
              <span className="text-[10px] uppercase font-bold text-zinc-700">Carregando Imagem...</span>
          </div>
        )}
        <div className="absolute top-0 left-0 bg-red-600 text-white text-[10px] font-bold px-3 py-1 z-10 shadow-lg">#{String(index + 1).padStart(2, '0')}</div>
        <div className="absolute top-2 right-2 text-white opacity-0 group-hover:opacity-100 transition-opacity z-10 drop-shadow-md"><GripVertical size={20} /></div>
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex justify-between items-start mb-3 min-h-[24px]">
            <h3 className={`font-bold truncate pr-2 uppercase tracking-tight ${data.title ? 'text-white' : 'text-zinc-700'}`}>{data.title || "SEM TÍTULO"}</h3>
        ...
        <div className="mt-auto pt-4 border-t border-zinc-900">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                   {data.image_base64 && <a href={data.image_base64} download={downloadName} className="text-zinc-600 hover:text-red-500 transition flex items-center gap-1 text-[10px] uppercase tracking-widest font-bold" title="Baixar Original" onClick={(e) => e.stopPropagation()}><Download size={12} /> <span className="hidden sm:inline">Download</span></a>}
                </div>
...
export default function App() {
  ...
  // --- BUSCA OTIMIZADA COM CHUNKS ---
  const fetchImagesForFrames = async (framesMetadata) => {
    if (!supabase) return;
    const CHUNK_SIZE = 3; // Baixa 3 imagens por vez para não dar timeout
    for (let i = 0; i < framesMetadata.length; i += CHUNK_SIZE) {
        const chunk = framesMetadata.slice(i, i + CHUNK_SIZE);
        const ids = chunk.map(f => f.id);
        const { data, error } = await supabase.from('sbframes').select('id, image_base64').in('id', ids);
        
        if (data) {
            setFrames(prevFrames => {
                const newFrames = [...prevFrames];
                data.forEach(item => {
                    const index = newFrames.findIndex(f => f.id === item.id);
                    if (index !== -1) newFrames[index] = { ...newFrames[index], image_base64: item.image_base64 };
                });
                return newFrames;
            });
        }
  ...
  useEffect(() => {
    ...
    // Carrega só metadados primeiro (super rápido)
    fetchFramesMetadata();
    const channel = supabase.channel(`sbframes_${currentProject.id}`)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'sbframes', filter: `project_id=eq.${currentProject.id}` }, () => fetchFramesMetadata(false))
        .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [currentProject, supabase]);

  const fetchFramesMetadata = async (showLoading = true) => {
    if (!currentProject) return;
    if (showLoading) setLoading(true);
    try {
      // 1. Busca LEVE (sem imagem)
      const { data, error } = await supabase
        .from('sbframes')
        .select('id, title, description, prompt, camera_move, order_index, created_at, project_id') // SEM image_base64
      ...
      setFrames(data || []); // Mostra os cards vazios/loading
      setErrorMsg(null);
      
      // 2. Dispara busca das imagens em background
      if (data && data.length > 0) {
          fetchImagesForFrames(data);
      }
  ...
  // ... [CÓDIGO DE CRUD E DRAG AND DROP - MANTIDO IGUAL] ...
  ...
  const handleSetCover = async (frame) => { if (!supabase || !currentProject) return; if (window.confirm("Definir este frame como capa do projeto?")) { try { const { error } = await supabase.from('sbprojects').update({ cover_image: frame.image_base64 }).eq('id', currentProject.id); if (error) throw error; alert("Capa atualizada com sucesso!"); } catch (error) { alert("Erro ao definir capa: " + error.message); } } };
  ...
  const handleSaveFrame = async (formData) => { if (!supabase || !currentProject) return; if (!formData.image && !formData.imagePreview) { alert("A imagem é obrigatória."); return; } setIsSaving(true); try { let imageBase64 = formData.imagePreview; if (formData.image) imageBase64 = await convertFileToBase64(formData.image); const newOrderIndex = frames.length; const frameData = { title: formData.title || '', description: formData.description || '', prompt: formData.prompt || '', camera_move: formData.cameraMove || '', image_base64: imageBase64, project_id: currentProject.id, order_index: editingFrame ? editingFrame.order_index : newOrderIndex, updated_at: new Date() }; if (editingFrame) { const { error } = await supabase.from('sbframes').update(frameData).eq('id', editingFrame.id); if (error) throw error; } else { const { error } = await supabase.from('sbframes').insert([frameData]); if (error) throw error; } setIsEditorOpen(false); setEditingFrame(null); fetchFramesMetadata(); } catch (error) { console.error("Erro ao salvar:", error); if (error.message && error.message.includes('413')) alert("Erro: Imagem muito grande."); else alert("Erro ao salvar: " + error.message); } finally { setIsSaving(false); } };
  const handleDeleteFrame = async (id) => { if (!supabase) return; if (window.confirm("Excluir este frame?")) { const { error } = await supabase.from('sbframes').delete().eq('id', id); if (error) alert("Erro ao deletar: " + error.message); fetchFramesMetadata(); } };
  ...
  const handleDropFile = async (e) => { e.preventDefault(); setIsDraggingFile(false); if (!e.dataTransfer.files || e.dataTransfer.files.length === 0) return; const files = Array.from(e.dataTransfer.files).filter(file => file.type.startsWith('image/')); if (files.length === 0) { alert("Apenas arquivos de imagem são permitidos."); return; } await uploadFilesBatch(files); };
  const uploadFilesBatch = async (files) => { if (!supabase || !currentProject) return; let successCount = 0; let failCount = 0; let currentOrderIndex = frames.length; for (let i = 0; i < files.length; i++) { setUploadProgress({ current: i + 1, total: files.length }); const file = files[i]; try { const imageBase64 = await convertFileToBase64(file); const fileName = file.name.replace(/\.[^/.]+$/, "").replace(/[-_]/g, " "); const frameData = { title: fileName, description: '', prompt: '', camera_move: '', image_base64: imageBase64, project_id: currentProject.id, order_index: currentOrderIndex + i, updated_at: new Date() }; const { error } = await supabase.from('sbframes').insert([frameData]); if (error) { console.error(`Erro ao enviar ${file.name}:`, error); failCount++; if (error.message && error.message.includes('413')) alert(`Imagem ${file.name} muito grande (limite ~6MB). Pulando...`); } else { successCount++; } } catch (err) { console.error(err); failCount++; } } setUploadProgress(null); if (successCount > 0) fetchFramesMetadata(false); if (failCount > 0) alert(`${failCount} imagens falharam ao enviar.`); };

  // --- RENDERIZAÇÃO ---
  if (!isLibLoaded || (loading && !projects.length && !currentProject)) {
    return <div className="min-h-screen bg-black flex flex-col items-center justify-center gap-4 text-zinc-500"><Loader2 className="animate-spin text-red-600" size={48} /><p className="text-xs uppercase tracking-widest text-red-600">Carregando BrickBoard...</p>{errorMsg && <p className="text-red-500 text-xs text-center px-4 max-w-md">{errorMsg}</p>}</div>;
  }
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-red-600 selection:text-white relative" onDragOver={handleDragOverFile} onDragLeave={handleDragLeaveFile} onDrop={handleDropFile}>
      ...
      <header className="sticky top-0 z-20 bg-black/90 backdrop-blur border-b border-zinc-900 px-8 py-6 flex justify-between items-center">
        <div className="flex items-center gap-6">
          <button onClick={() => setCurrentProject(null)} className="p-2 text-zinc-500 hover:text-white transition" title="Voltar"><ChevronLeft size={24} /></button>
          <div className="flex flex-col"><h1 className="text-2xl font-bold tracking-tight text-white leading-none uppercase">{currentProject.title}</h1><span className="text-[10px] font-mono text-zinc-500 mt-2 uppercase tracking-widest">BrickBoard Story System</span></div>
        </div>
        <div className="flex items-center gap-4">
           ...
           <div className="hidden md:flex bg-zinc-950 border border-zinc-800"><button onClick={() => setViewMode('grid')} className={`p-3 ${viewMode === 'grid' ? 'bg-red-600 text-white' : 'text-zinc-500 hover:text-white'}`}><Maximize2 size={18} /></button><button onClick={() => setViewMode('list')} className={`p-3 ${viewMode === 'list' ? 'bg-red-600 text-white' : 'text-zinc-500 hover:text-white'}`}><Move size={18} /></button></div>
           <button onClick={() => { setEditingFrame(null); setIsEditorOpen(true); }} className="bg-red-600 hover:bg-red-700 text-white px-6 py-3 font-bold flex items-center gap-2 transition text-xs uppercase tracking-widest shadow-[0_0_10px_rgba(220,38,38,0.4)]"><Plus size={16} /> <span className="hidden md:inline">Novo Frame</span></button>
        </div>
      </header>

      <main className="p-8 max-w-[1800px] mx-auto min-h-[calc(100vh-100px)]">
        {loading ? (
            <div className="flex flex-col items-center justify-center h-[60vh] gap-6"><div className="relative"><div className="absolute inset-0 bg-red-600 blur-xl opacity-20 rounded-full animate-pulse"></div><Loader2 className="animate-spin text-red-600 relative z-10" size={64} /></div><p className="text-zinc-500 text-xs font-bold uppercase tracking-[0.2em] animate-pulse">Carregando Storyboard...</p></div>
        ) : frames.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-40 border border-dashed border-zinc-900 bg-zinc-950"><div className="w-20 h-20 bg-zinc-900 flex items-center justify-center mb-6"><Film size={40} className="text-zinc-700" strokeWidth={1} /></div><h2 className="text-xl font-bold text-white mb-2 uppercase tracking-widest">Projeto Vazio</h2><p className="text-zinc-600 text-sm mb-4 text-center max-w-md">Arraste seus frames para cá ou adicione manualmente.</p>{errorMsg && (<div className="bg-red-500/10 border border-red-500/50 text-red-500 p-3 rounded mb-4 text-xs font-mono max-w-md text-center">{errorMsg}</div>)}<div className="flex gap-4"><button onClick={() => fetchFramesMetadata(true)} className="bg-zinc-800 text-white px-6 py-3 font-bold uppercase tracking-widest hover:bg-zinc-700 transition flex items-center gap-2"><RefreshCw size={16} /> Recarregar</button><button onClick={() => { setEditingFrame(null); setIsEditorOpen(true); }} className="bg-red-600 text-white px-8 py-3 font-bold uppercase tracking-widest hover:bg-red-700 transition">Adicionar Frame Inicial</button></div></div>
        ) : (
          <div className={`grid gap-8 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5' : 'grid-cols-1'}`}>
            {frames.map((frame, index) => (
              <FrameCard key={frame.id} index={index} data={frame} onDelete={handleDeleteFrame} onEdit={(f) => { setEditingFrame(f); setIsEditorOpen(true); }} onSetCover={handleSetCover} onDragStart={handleDragStart} onDragEnter={handleDragEnter} onDragEnd={handleDragEnd} />
            ))}
